...
# Low-level.
apt install -y man-db aptitude neofetch git curl wget vim emacs-nox sudo p7zip-full zip unzip scanmem strace lsof htop screen tmux nmap build-essential tshark gdisk tftp ftp tcpdump transmission-cli transmission-daemon net-tools nethogs iftop software-properties-common ntp lm-sensors hddtemp tree ufw
